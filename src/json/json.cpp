...
namespace Json {
    ...
    JsonObject JsonObject::getObject(const QString &key)
    {
        return JsonObject(m_data[key].toMap());
    }

    JsonArray JsonObject::getArray(const QString &key)
    {
        return JsonArray(m_data[key].toList());
    }

    QString JsonObject::getString(const QString &key)
    {
        return m_data.value(key).toString();
    }

    int JsonObject::getInt(const QString &key)
    {
        return m_data.value(key).toInt();
    }

    bool JsonObject::getBool(const QString &key)
    {
    ...
    JsonArray::JsonArray(const QVariantList &src)
    {
        m_data = src;
    }
    ...
    JsonObject JsonArray::getObject(int index)
    {
        if(index < m_data.size())
            return JsonObject(m_data.at(index).toMap());
        else
            return JsonObject();
    }
    ...
    {
        if(index < m_data.size())
            return JsonArray(m_data.at(index).toList());
        else
            return JsonArray();
    }

    QString JsonArray::getString(int index)
    {
        return m_data.at(index).toString();
    }

    int JsonArray::getInt(int index)
    {
        return m_data.at(index).toInt();
    }

    bool JsonArray::getBool(int index)
    {
        return m_data.at(index).toBool();
    }
